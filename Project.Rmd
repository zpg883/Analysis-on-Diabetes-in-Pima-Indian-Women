...
knn.conf = confusionMatrix(data = predict(knn.model, diabetesTest[-9]), reference = diabetesTest$Outcome)
knn.conf
```
...
Model results using test data.
```{r, echo=FALSE}
rf.conf = confusionMatrix(data = predict(rf.fit, diabetesTest[-9]), reference = diabetesTest$Outcome)
rf.conf
```
...
Model results using test data.
```{r, echo=FALSE}
log.conf = confusionMatrix(data = predict(logisticTune, diabetesTest[-9]), reference = diabetesTest$Outcome)
log.conf
```
