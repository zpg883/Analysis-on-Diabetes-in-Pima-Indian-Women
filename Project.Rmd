...
SVM round 2 - Removing Insulin and Skin Thickness (doesn't make a big difference)
```{r, echo=FALSE}
diabetesTrain_minusInsulinSkin = diabetesTrain[-4]
diabetesTrain_minusInsulinSkin = diabetesTrain_minusInsulinSkin[-4]

diabetesTest_minusInsulinSkin = diabetesTest[-4]
diabetesTest_minusInsulinSkin = diabetesTest_minusInsulinSkin[-4]
...
sigmaRangeReduced <- sigest(as.matrix(diabetesTrain_minusInsulinSkin[-7]))
svmRGridReduced <- expand.grid(.sigma = sigmaRangeReduced[1],
                               .C = 2^(seq(-4, 4)))

SVMTune_minuscolumns <- train(x = diabetesTrain_minusInsulinSkin[-7],
                 y = diabetesTrain_minusInsulinSkin$Outcome,
                 method = "svmRadial", 
                 metric = "Accuracy",
                 preProc = c("center", "scale"),
                 tuneGrid = svmRGridReduced,
                 fit = FALSE,
                 trControl = ctrl)
SVMTune_minuscolumns
```


```{r, echo=FALSE}
set.seed(100)
confusionMatrix(data = predict(SVMTune_minuscolumns, diabetesTest_minusInsulinSkin[-7]), reference = diabetesTest_minusInsulinSkin$Outcome)
```
...
Checking out resamples just to see
```{r}
res = resamples(list(Logistic = logisticTune, SVM = SVMTune, RF=rf.fit, KNN=knn.model, SVM_WithoutInsulinSkin=SVMTune_minuscolumns))
dotplot(res)
```
